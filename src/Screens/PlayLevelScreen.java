...
public class PlayLevelScreen extends Screen {
    ...
    public void update() {
        ...
        */
        if (map.getFlagManager().isFlagSet("house1ToTown")) {
            map = townMap;
            point = map.getPositionByTileIndex(17, 24);
            player.setMap(map);
        ...
        if (map.getFlagManager().isFlagSet("townToHouse1")) {
            map = house1Map;
            point = map.getPositionByTileIndex(17, 21); //6,4
            player.setMap(map);
            player.setLocation(point.x, point.y);
            player.setFacingDirection(Direction.RIGHT);
